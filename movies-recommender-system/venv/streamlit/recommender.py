...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("<p>Set of Instructions to be followed</p>",
            unsafe_allow_html=True)
st.markdown("""
...
selected_movie = st.sidebar.selectbox(
    'Type or select a movie from the dropdown', df_values_title)


def recommender(movie_name):
    movie_index = df[df.title == movie_name].index[0]
    movie_top_10 = sorted(list(enumerate(
        SimilarityScore[movie_index])), reverse=True, key=lambda x: x[1])[1:10]
    recommended = []
    recommend_id = []
    recommend_poster = []
    rec_des = []
...
if st.sidebar.button('Recommend'):
    ...
    st.sidebar.image(fetch_data_poster(df.loc[movie_index].movie_id))
    st.sidebar.write(fetch_data_genre(df.loc[movie_index].movie_id)[0]['name'])
    st.sidebar.write(fetch_data_describe(df.loc[movie_index].movie_id))
    # st.image()
    ...
    for i in range(0, 9):
        if i % 2 == 0:
            col1.markdown("<p style=\"font-size:28px\">" +
                          recommendations[i]+"</p>", unsafe_allow_html=True)
            col1.image(posters[i])
            col1.write("Genre:{}".format(genres[i][0]['name']))
            col1.write('{}...'.format(describe[i])[:300])
        else:
            col2.markdown("<p style=\"font-size:28px\">" +
                          recommendations[i]+"</p>", unsafe_allow_html=True)
